...
const char *uiPhonebookStatusMessage(PhonebookStatus status)
{
    ...
    case PHONEBOOK_ID_SPACE_EXHAUSTED:
        return "свободные ID закончились";
    case PHONEBOOK_RESULT_TOO_SMALL:
        return "буфер результатов слишком мал";
    default:
        return "неизвестная ошибка справочника";
    }
}

static void printPhonebookFailure(PhonebookStatus status)
{
    printf("Ошибка: %s.\n", uiPhonebookStatusMessage(status));
}
...
static PhonebookStatus deleteContact(Phonebook *phonebook)
{
    ...
    if (!readInt("ID контакта: ", &id))
        return PHONEBOOK_INVALID_ARGUMENT;
    status = phonebookRemove(phonebook, id);
    if (status == PHONEBOOK_OK)
...
PhonebookStatus uiRun(Phonebook *phonebook, const char *contacts_file)
{
    ...
    while (choice != 6) {
        ...
        if (status != PHONEBOOK_OK)
            printPhonebookFailure(status);
        if (changed) {
            StorageStatus storage_status =
                storageSave(phonebook, contacts_file);
